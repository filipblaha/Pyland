...
class Text(pygame.sprite.Sprite):
    ...
    def render_user_text(self):
        for i, row in enumerate(self.user_text):
            ...
            self.screen.blit(text_surf, (self.text_x, len(self.preset_text) * self.preset_text_height + self.text_y + i * text_height))

    def blink_cursor(self):
        cursor_rect = pygame.Rect(self.text_x + self.f.size(self.user_text[self.cursor_row][:self.cursor_index])[0], len(self.preset_text) * self.preset_text_height + self.text_y + self.cursor_row * 31, 2, 30)  # Cursor
